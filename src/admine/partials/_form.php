...
<div class="flex gap-3 md:w-2/3">
  <input
    value="<?php echo isset($player['player_name']) ? $player['player_name'] : "" ?>"
  ...
  name="flag" id="select-nation">
    <option class="text-gray-400" selected disabled value="">Nationalité</option>
        <?php while ($row_Nationality = $result_Nationality->fetch_assoc()): ?>
            ...
            <option <?php echo isset($player['N_id']) && $player['N_id'] == $row_Nationality['N_id'] ? "selected" : "" ?> value="<?php echo $row_Nationality['N_id']; ?>"><?php echo $row_Nationality['N_Name']; ?></option>
            <?php } else { ?>
      ...
      <select
        ...
        <?php while ($row_Club = $result_Club->fetch_assoc()): ?>
          <img src="<?php echo $row_Club['team_image'] ?>" alt="">
                  <?php if ($row_Club > 0) { ?>
                    <option <?php echo isset($player['team_id']) && $player['team_id'] == $row_Nationality['team_id'] ? "selected" : "" ?> value="<?php echo $row_Club['team_id'] ?>"><?php echo $row_Club['team_name'] ?></option>
                    <?php } else { ?>
                      <option value="">Aucun joueur sélectionné</option>
                    <?php }  ?>
          <?php endwhile ?>
...
<div class="flex gap-3 md:w-2/3">
  <input
    value="<?php echo isset($player['player_rating']) ? $player['player_rating'] : "" ?>"
    class="border-gray-200 focus:border-gray-700 bg-gray-200 px-4 py-2 rounded w-full text-gray-700 leading-tight appearance-none focus:outline-none"
...
<div id="player_carater" class="gap-3 grid">
  <div class="flex gap-3 md:w-2/3">
    <input
    value="<?php echo isset($player['player_pace']) ? $player['player_pace'] : '' ?>"
      class="border-gray-200 focus:border-gray-700 bg-gray-200 px-4 py-2 rounded w-full text-gray-700 leading-tight appearance-none focus:outline-none"
      type="number" name="pace" placeholder="pace">
    <input
    value="<?php echo isset($player['player_shooting']) ? $player['player_shooting'] : '' ?>"
      class="border-gray-200 focus:border-gray-700 bg-gray-200 px-4 py-2 rounded w-full text-gray-700 leading-tight appearance-none focus:outline-none"
      type="number" name="shooting" placeholder="shooting">
  </div>
  <div class="flex gap-3 md:w-2/3">
    <input
        value="<?php echo isset($player['player_passing']) ? $player['player_passing']  : '' ?>"
        class="border-gray-200 focus:border-gray-700 bg-gray-200 px-4 py-2 rounded w-full text-gray-700 leading-tight appearance-none focus:outline-none"
        type="number" name="passing" placeholder="passing">
    <input
        value="<?php echo isset($player['player_dribbling']) ? $player['player_dribbling'] : ''  ?>"
        class="border-gray-200 focus:border-gray-700 bg-gray-200 px-4 py-2 rounded w-full text-gray-700 leading-tight appearance-none focus:outline-none"
        type="number" name="dribbling" placeholder="dribbling">
  </div>
  <div class="flex gap-3 md:w-2/3">
    <input
        value="<?php echo isset($player['player_defending']) ? $player['player_defending'] : ''  ?>"
        class="border-gray-200 focus:border-gray-700 bg-gray-200 px-4 py-2 rounded w-full text-gray-700 leading-tight appearance-none focus:outline-none"
        type="number" name="defending" placeholder="defending">
    <input
        value="<?php echo isset($player['player_physical']) ? $player['player_physical'] : '' ?>"
        class="border-gray-200 focus:border-gray-700 bg-gray-200 px-4 py-2 rounded w-full text-gray-700 leading-tight appearance-none focus:outline-none"
